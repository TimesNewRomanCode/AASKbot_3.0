...
from app.services.services_for_models.sender_logs import (
    quick_create_log,
    get_has_today_log,
)
...
async def send_schedules():
    ...
    try:
        ...
        if result_log:
            async with AsyncSessionLocal() as session:
                ...
                for user in users:
                    ...
                    file_path = f"./app/grop_photo/{user.college_name}/{user.address_name}/{day_month}/{user.group_name}.png"
        ...
        print(
            f"Отправлено: {users_found}, Не найдено: {users_not_found}, Время: {execution_time}"
        )
...
async def try_send_until_success(retry_interval=600):
    ...
    while True:
        ...
        print(
            f"Файлы расписания не найдены. Следующая попытка через {retry_interval // 60} минут..."
        )
        attempt += 1
...
async def scheduled_task():
    ...
    if now.time() < start_time_today:
        ...
        print(
            f"Ожидание начала отправки: {wait_seconds // 3600:.0f}ч {(wait_seconds % 3600) // 60:.0f}м"
        )
        await asyncio.sleep(wait_seconds)
